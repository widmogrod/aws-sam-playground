...
```bash
sam-quick-start-step$ sam build
```
...
```bash
sam-quick-start-step$ sam logs -n StockCheckerFunction --stack-name sam-quick-start-step --tail
```
...
```bash
sam-quick-start-step$ go test -v ./functions/...
```
...
```bash
aws cloudformation delete-stack --stack-name sam-quick-start-step
```
